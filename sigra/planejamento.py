...
class Oferta:
    @staticmethod
    def OFELIST(in_file, encoding='utf-16'):
        ...
        while i < num_lines:
            ...
            if eh_disciplina(line):
                codigo, nome = parse_disciplina(line)
                oferta[codigo] = {'nome': nome}

                # ### Pré-requisitos ###
                i += 1
                line = content[i]
                oferta[codigo]['créditos'] = parse_creditos(line)

                pre_reqs = ''
                while not line.startswith('   Turma'):
                    pre_reqs += parse_pre_requisitos(line)
                ...
                while i < num_lines:
                    ...
                    if not eh_nova_turma(line):
                        break

                    (t, descricao, vagas, turno,
                     aula, professor, reserva, obs) = parse_turma(line)
                    ...
                    while i < num_lines:
                        ...
                        if eh_nova_turma(line) or eh_disciplina(line):
                            break
